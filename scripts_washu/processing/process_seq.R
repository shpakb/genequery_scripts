...
ct <- as.character(args[4])
logScaleF <- as.logical(args[5])
quantileF <- as.logical(args[6])

# Outs needed
expMatF <- as.logical(args[7])
modulesF <- as.logical(args[8])
eigengenesF <- as.logical(args[9])
svgF <- as.logical(args[10])
...
tryCatch({
  ...
  {
    exp[,2:ncol(exp)] <-
      normalizeBetweenArrays(exp[,2:ncol(exp)], method = "quantile")
  }
  
  exp <-
    collapseRows(exp[,2:ncol(exp)], exp$entrez, rownames(exp))
